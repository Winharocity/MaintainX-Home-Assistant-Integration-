    ...
    return [
        {
            ...
            "action": [
                ...
                {"service": "persistent_notification.create", "data": {"title": "MaintainX", "message": "✅ Work order created successfully!"}},
            ],
            "mode": "single",
        },
        _quick_report_automation("maintainx_quick_report_printer", "input_button.maintainx_report_printer", "Printer Issue Reported", "Printer issue reported via HA dashboard", "MEDIUM", "CORRECTIVE", "🖨️ Printer issue reported!"),
...
def _quick_report_automation(
    ...
    return {
        ...
        "description": f"Quick report: {title}",
        ...
        "action": [
            {
                ...
                "data": {
                    "title": title,
                    "description": description + " at {{ now().strftime('%Y-%m-%d %H:%M:%S') }}",
                    "priority": priority,
                    "category": category,
                },
            },
            {"service": "persistent_notification.create", "data": {"title": "MaintainX", "message": notification_msg}},
        ],
        "mode": "single",
    }
